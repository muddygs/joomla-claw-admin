...
class DbBlobCacheWriter
{
   ...
   * @param string $columnName Column name containing the blobs
   ...
   * @throws Exception 
   */
  public function save(string $tableName, array $rowIds, string $columnName, array $minAges = []): array
  {
    $this->createCacheDir();
    $filenames = [];
    ...
    foreach ($rowIds as $rowId) {
      if ( $this->extension != '' ) {
        ...
        $blobData = $this->getBlobData($tableName, $rowId, $columnName);
  ...
  private function getBlobData($tableName, $rowId, $columnName): ?string
  {
  ...
  private function getMimeExtension(string $mimeType): ?string
  {
    ...
    foreach ($lines as $line) {
      ...
      // Ignore entries with explicitly defined extensions
      if (count($parts) < 2) {
        continue;
      }
