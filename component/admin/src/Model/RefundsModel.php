...
class RefundsModel extends FormModel
{
  ...
  private function formatTransactionRow(int $index, int $id, int $state, string $title, string $invoice_number,
      string $register_date, int $transaction_id, string $regCode, int $profileId, float $amount, string $info) {
    $colName = "col-6 col-lg-3";
    $colInvoice = "col-5 col-lg-2";
    $colRegDate = "col-4 col-lg-2";
    $colTransId = "col-4 col-lg-2";
  ...
  function refundChargeProfile(Json $json)
  {
    ...
    if ( $rawRegistrantRow != null ) {
      ...
      if ( $alias !== false ) {
        ...
        list($status, $transId, $errorMsg) = $profile->chargeCustomerProfile($record, 
